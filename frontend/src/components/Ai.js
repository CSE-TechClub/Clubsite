...
const Ai = () => {
  ...
  const fullText = 'AI Avengers'; // Space between "AI" and "Avengers"
  const typingSpeed = 150; // milliseconds per character
  const typingRef = useRef(null);
  const cursorTimerRef = useRef(null);
  const sectionRef = useRef(null);
  const animationPlayedRef = useRef(false); // Moved outside the effect
  ...
  return (
    <section className="club-section ai-section" ref={sectionRef}>
